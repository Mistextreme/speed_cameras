...
if Config.Targets_mobile.System ~= 'ox_target' then
    return
end
...
Citizen.CreateThread(function()
    while not exports.ox_target do Citizen.Wait(500) end
    ...
    if Config.SirenImmunityWhitelist and Config.SirenImmunityWhitelist.police then
        for _, modelName in ipairs(Config.SirenImmunityWhitelist.police) do
            table.insert(policeVehicles, GetHashKey(modelName))
        end
    end
    ...
    exports.ox_target:addModel(policeVehicles, {
        {
            name = 'get_mobile_radar_trunk',
            label = 'Take the mobile radar',
            icon = 'fa-solid fa-camera-retro',
            bone = 'trunk',
            distance = 1.5,
            onSelect = function(data)
                PlayTrunkAnimation(data.entity, function()
        ...
        {
            name = 'return_mobile_radar_trunk',
